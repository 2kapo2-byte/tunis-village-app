...
class AvailabilityRepository {
  ...
  Future<List<AvailableUnit>> search(SearchQuery query) async {
    if (query.nights <= 0) {
      throw const FormatException('Check-out must be after check-in.');
    }
    if (query.guests.adults < 1) {
    ...
    final propertyRows = await _client
        .from('properties')
        .select('id, name, location, description, min_stay, max_stay, status')
        .inFilter('id', propertyIds)
        .eq('status', 'approved');

    final properties = <String, Map<String, dynamic>>{
      for (final row in (propertyRows as List).whereType<Map<String, dynamic>>())
        row['id'].toString(): row,
    };
    ...
    final eligibleUnits = units.where((row) {
      ...
      return capacity >= query.guests.totalGuests &&
          query.nights >= minStay &&
          (maxStay <= 0 || query.nights <= maxStay);
    }).toList(growable: false);

    if (eligibleUnits.isEmpty) return const [];
    ...
    final availabilityRows = await _client
        ...
        .gte('date', _date(query.checkIn))
        .lt('date', _date(query.checkOut));

    final blockedUnitIds = <String>{};
    for (final row in (availabilityRows as List).whereType<Map<String, dynamic>>()) {
      final status = row['status']?.toString();
      if (status == 'booked' || status == 'blocked' || status == 'external') {
        blockedUnitIds.add(row['unit_id'].toString());
      }
    }

    return List.unmodifiable(
      eligibleUnits
          .where((row) => !blockedUnitIds.contains(row['id'].toString()))
          .map((row) {
            final property = properties[row['property_id'].toString()]!;
            return AvailableUnit(
              ...
              minStay: (property['min_stay'] as num?)?.toInt() ?? 1,
              maxStay: (property['max_stay'] as num?)?.toInt() ?? 0,
            );
          }),
    );
  }
