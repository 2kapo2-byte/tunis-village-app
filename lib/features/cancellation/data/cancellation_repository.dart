...
class CancellationResult {
  ...
  factory CancellationResult.fromRpc(Object? value) {
    ...
    if (map['success'] == false || map['error'] != null) {
      throw StateError((map['error'] ?? 'cancellation_failed').toString());
    }
...
class CancellationRepository {
  ...
  Future<CancellationResult> cancel({required String bookingId, String? reason}) async {
    final result = await _client.rpc('cancel_own_booking', params: {
      'p_booking_id': bookingId,
      'p_reason': reason,
    });
    return CancellationResult.fromRpc(result);
  }

  Future<RefundStatus?> getLatestRefund(String bookingId) async {
    final rows = await _client
        .from('refunds')
        .select('id, amount, status, reason, provider_reference')
